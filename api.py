...
def configure_ngrok():
    ngrok_token = "2pOtIPb7nDjMhjWcnRtf74w2aDM_iv5FakdkujArDYnRkboe"
    try:
...
def update_soul_txt(public_url):
    with open("babu1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in babu1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("babu1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated babu1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update babu1.txt on VPS: {str(e)}")
